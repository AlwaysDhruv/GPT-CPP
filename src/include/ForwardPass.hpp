...
namespace Forward
{
	...
	vector<vector<float>> Layers(mINI::INIStructure& ini, auto X_IN, auto& A, auto& H, auto& z, auto& at , auto& score, const auto& w_query, const auto& w_key, const auto& w_value, const auto& w_output, const auto& w1, const auto& w2, const auto& embbed_matrix, const auto& b1, const auto& b2, const auto& b_lm, auto& qry, auto& ky, auto& val, auto& X_IN2, auto& X_IN3)
	{
		...
		{
			auto X_NORM = Functions::normalization(X_IN);
			...
			//FFN
			X_NORM = Functions::normalization(X_NEW);
		...
		X_IN = Functions::normalization(X_IN);

		H = X_IN;

		auto embbed_matrix_t = Tensor::transpose(embbed_matrix);
		
		X_IN = Tensor::dot_product(X_IN, embbed_matrix_t);
